function  M = computeM(itr,m,n,u,img,W,opts)
 
%   u = img ; % this is the ideal case,the implemetation of IISD-SB method;
%   
    frame = opts.frame ;
    C = W(u);
    Level=length(C);
...
for ki=1:Level 
    for ji=1:nD
        for jj=1:nD           
            logical{ki}{ji,jj} = abs(C{ki}{ji,jj})>= threshold/3^(itr+1);
            switch itr
                case 1     
